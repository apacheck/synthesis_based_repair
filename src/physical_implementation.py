...
def spoof_elaborateDMP(old_skill, new_skill, suggestion, hard_constraints, symbols, workspace_bnds, opts):
    intermediate_sat = []
    val_loss = 1
    transitions = suggestion['intermediate_states']
    for transition in transitions:
        # print(transition[0])
        # print(transition[1][0])
        post_sat = []
        for post in transition[1]:
            x_same = (transition[0]['x0'] and post['x0']) or (transition[0]['x1'] and post['x1']) or (transition[0]['x2'] and post['x2'])
            y_same = (transition[0]['y0'] and post['y0']) or (transition[0]['y1'] and post['y1']) or (transition[0]['y2'] and post['y2'])
            # z_same = (transition[0]['z0'] and post['z0']) or (transition[0]['z1'] and post['z1']) or (transition[0]['z2'] and post['z2'])
            # z_same = (transition[0]['z0'] and post['z0']) or (transition[0]['z1'] and post['z1'])
            y_change = (transition[0]['y0'] and post['y1']) or (transition[0]['y1'] and post['y0']) or (transition[0]['y2'] and post['y1']) or (transition[0]['y1'] and post['y2'])
            x_change = (transition[0]['x0'] and post['x1']) or (transition[0]['x1'] and post['x0']) or (transition[0]['x2'] and post['x1']) or (transition[0]['x1'] and post['x2'])
            # z_change = (transition[0]['z0'] and post['z1']) or (transition[0]['z1'] and post['z0']) or (transition[0]['z2'] and post['z1']) or (transition[0]['z1'] and post['z2'])
            # z_change = (transition[0]['z0'] and post['z1']) or (transition[0]['z1'] and post['z0'])
            if not ((x_change and y_same) or (x_same and y_change)):
                val_loss = 0
                post_sat.append(0)
            else:
                post_sat.append(1)
        intermediate_sat.append(np.array([[np.NaN, np.NaN, np.NaN, max(post_sat)]]))
    val_losses = [np.array([np.NaN, np.NaN, np.NaN, val_loss])]

    return None, val_losses, intermediate_sat


def spoof_elaborateDMP_find(old_skill, new_skill, suggestion, hard_constraints, symbols, workspace_bnds, opts):
    intermediate_sat = []
    val_loss = 1
    transitions = suggestion['intermediate_states']
    for transition in transitions:
...
def spoof_elaborateDMP_baxter(old_skill, new_skill, suggestion, hard_constraints, symbols, workspace_bnds, opts):
    intermediate_sat = []
    val_loss = 1
    transitions = suggestion['intermediate_states']
    for transition in transitions:
        ...
        post_sat = []
        for post in transition[1]:
            leftupdown = (transition[0]['leftbottom'] and post['lefttop']) or (transition[0]['lefttop'] and post['leftbottom'])
            rightupdown = (transition[0]['rightbottom'] and post['righttop']) or (transition[0]['righttop'] and post['rightbottom'])
            leftfront = (transition[0]['lefttop'] and post['front']) or (transition[0]['leftbottom'] and post['front']) or (transition[0]['front'] and post['lefttop']) or (transition[0]['front'] and post['leftbottom'])
            rightfront = (transition[0]['righttop'] and post['front']) or (transition[0]['rightbottom'] and post['front']) or (transition[0]['front'] and post['righttop']) or (transition[0]['front'] and post['rightbottom'])
            leftback = (transition[0]['lefttop'] and post['back']) or (transition[0]['leftbottom'] and post['back']) or (transition[0]['back'] and post['lefttop']) or (transition[0]['back'] and post['leftbottom'])
            rightback = (transition[0]['righttop'] and post['back']) or (transition[0]['rightbottom'] and post['back']) or (transition[0]['back'] and post['righttop']) or (transition[0]['back'] and post['rightbottom'])
            if not (leftupdown or rightupdown or leftfront or rightfront or leftback or rightback):
                val_loss = 0
                post_sat.append(0)
            else:
                post_sat.append(1)

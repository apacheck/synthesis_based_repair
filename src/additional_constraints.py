...
# import src.additional_ltldiff as ltd2
import torch
import numpy as np
        ...
        for corner_x, corner_y in corners:
            ...
            for sym_name, sym in self.symbols.items():
                ...
                if sym.get_type() == 'circle':
                    ...
                    #     ltd.LT2(
                    #         ltd.TermStatic(torch.norm(point.x - torch.from_numpy(sym['center']), dim=1, keepdim=True)),
                    #         ltd.TermStatic(torch.from_numpy(sym['radius'])), dim=[0])
                    # )
        ...
        for sym_name, sym in self.symbols.items():
            ...
            if sym.get_type() == 'circle':
                bnds_list.append(ltd.LT2(
                    ...
                    ltd.TermStatic(sym.get_radius()), dim=np.array([0])))
            # for dim in sym['dims']:
            #     bnds_list.append(ltd.GEQ2(rollout_term, ltd.TermStatic(sym['bnds'][[dim], 0]), dim=np.array([dim])))
            #     bnds_list.append(ltd.LEQ2(rollout_term, ltd.TermStatic(sym['bnds'][[dim], 1]), dim=np.array([dim])))
            sym_ltd[sym_name] = ltd.And(bnds_list)
        neg_sym_ltd = dict()
        for sym_name, sym in self.symbols.items():
            bnds_list = []
            ...
            if sym.get_type() == 'circle':
                bnds_list.append(ltd.GEQ2(
                    ...
                    ltd.TermStatic(sym.get_radius()), dim=np.array([0])))
            # for dim in sym['dims']:
            #     bnds_list.append(ltd.LT2(rollout_term, ltd.TermStatic(sym['bnds'][[dim], 0]), dim=np.array([dim])))
            #     bnds_list.append(ltd.GT2(rollout_term, ltd.TermStatic(sym['bnds'][[dim], 1]), dim=np.array([dim])))
            neg_sym_ltd[sym_name] = ltd.Or(bnds_list)
...
class States:
    def __init__(self, symbols, suggestions_pre, epsilon, buffer=0.1):
        self.symbols = symbols
        ...
        for sym_name, sym in self.symbols.items():
            ...
            if sym.get_type() == 'rectangle':
                for dim in sym.get_dims():
                    ...
                    #     ltd.GEQ2(point, ltd.TermStatic((torch.from_numpy(sym['bnds'][:, 0]) + self.buffer * (
                    #             torch.from_numpy(sym['bnds'][:, 1]) - torch.from_numpy(sym['bnds'][:, 0])))[[dim]]),
                    #                   dim=np.array([dim])),
                    #     ltd.LEQ2(point, ltd.TermStatic((torch.from_numpy(sym['bnds'][:, 1]) - self.buffer * (
                    #                      torch.from_numpy(sym['bnds'][:, 1]) - torch.from_numpy(sym['bnds'][:, 0])))[[dim]]),
                    #                   dim=np.array([dim]))]))
                    bnds_dim_list.append(ltd.And([
